...
function ENT:Initialize()
	local ecolor = gemstone.config.emeraldcolor

	self:SetModel("models/props_c17/consolebox01a.mdl")
	self:PhysicsInit(SOLID_VPHYSICS)
	self:SetMoveType(MOVETYPE_VPHYSICS)
	self:SetSolid(SOLID_VPHYSICS)
	self:SetColor( ecolor )
...
function ENT:CreateMoneybag()
	...
	local printamount = gemstone.config.emeraldprintamount
	local printtime = gemstone.config.emeraldprinttime
...
function ENT:Use(activator)
	...
	DarkRP.notify(activator, 1, 4, "You have collected $"..self:GetNWInt("PrintA").." from a Emerald Printer.")
	self:SetNWInt("PrintA",0)
	end
end
